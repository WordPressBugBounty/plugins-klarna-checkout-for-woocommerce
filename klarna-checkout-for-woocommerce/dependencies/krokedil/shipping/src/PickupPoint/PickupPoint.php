...
namespace KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\PickupPoint;

use KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\PickupPoint\Address;
use KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\PickupPoint\Coordinates;
use KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\PickupPoint\OpenHours;
use KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\PickupPoint\EstimatedTimeOfArrival;
use KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\Traits\ArrayFormat;
use KrokedilKlarnaCheckoutDeps\Krokedil\Shipping\Traits\JsonFormat;
/**
...
class PickupPoint
{
    ...
    public function set_from_array($pickup_point = array())
    {
        ...
        $this->set_address(\html_entity_decode($address['street'] ?? ''), \html_entity_decode($address['city'] ?? ''), \html_entity_decode($address['postcode'] ?? ''), \html_entity_decode($address['country'] ?? ''));
        $coordinates = $pickup_point['coordinates'] ?? array();
        $this->set_coordinates(\html_entity_decode($coordinates['latitude'] ?? ''), \html_entity_decode($coordinates['longitude'] ?? ''));
        $this->set_open_hours($pickup_point['open_hours'] ?? array());
        $eta = $pickup_point['eta'] ?? array();
        $this->set_eta(\html_entity_decode($eta['utc'] ?? ''), \html_entity_decode($eta['local'] ?? ''));
